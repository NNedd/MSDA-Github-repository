...
```{r cases-variales-and-datatypes, eval = TRUE}
dim(cdc)
str(cdc)
```
There are 20000 cases and 9 variables.  The type of data for each variable are:
  ...
  * wtdesicre - numerical/continuous
...
```{r summary-for-height, eval =TRUE}
summary(cdc$height)
```
```{r IQR-for-height, eval =TRUE}
70-64
```

```{r summary-for-weight, eval =TRUE}
summary(cdc$weight)
```
...
```{r prop-excellent-health, eval = TRUE}
...
_More females tend to not smoke and more males (by a small margin) tend to smoke_
...
_The boxplot shows that the healthier a person feels then on average their BMI is lower_

_I would find it interesting to see whether there was any relationship between BMI and gender. From looking at shows like the Biggest Loser, it seems as though the men loose weight at a faster pace - maybe there is a natural tendency for male BMI to be lower_

```{r bmi-gender, eval=TRUE}
bmi <- (cdc$weight / cdc$height^2) * 703
boxplot(bmi ~ cdc$gender)
```

_Interestingly, it seems as though females have a lower bmi on average, with the exception of one noticable outlier.  It should be noted that more men had a lower than average bmi._
...
_The both have similar shapes_
...
```{r height-vs-weight, eval = TRUE}
plot(cdc$height, cdc$weight)
```
_The correlation is tending towards a positive one - as height increases, weight increases._
...
```{r wdiffcalc, eval = TRUE}
wtdiff <- cdc$weight - cdc$wtdesire
```
...
_wtdiff is numberical data.  If the value is 0, it means that the person currently weighs their desired weight (they do not want to lose or gain weight).  If the value is negative is means that the person wants to gain weight.
...
```{r histwtdiff, eval = TRUE}
hist(wtdiff)
```
_This shows that most persons are within 50 pounds of their desired weight_

Using a boxplot:

```{r boxplotwtdiff, eval = TRUE}
boxplot(wtdiff)
```
_Again this shows that mean wtdiff is very close to 0 and there is not must variation in the values._


-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.

_The data on males is already in the mdata dataframe. The data on females will be placed in the fdata dataframe._

```{r females, eval=TRUE}
fdata <- subset(cdc, cdc$gender == "f")
```

_The weight difference will be calculated for both males and females_
```{r wdiffmfcalc, eval = TRUE}
wtdiffm <- mdata$weight - mdata$wtdesire
wtdifff <- fdata$weight - fdata$wtdesire
```

_Numercial Summaries_

```{r numerical-summaries, eval = TRUE}
summary(wtdiffm)

summary(wtdifff)
```

_On average men have a lower weight difference._

_Boxplots_

```{r boxplot-wtdiffm-wtdifff, eval=TRUE}
boxplot(wtdiffm)
boxplot(wtdifff)
```

_These box plots show that more men want to gain weight and more women want to loose weight_
...
```{r mean-sd-weight, eval = TRUE}
meanweight <- mean(cdc$weight)

sdweight <- sd(cdc$weight)

lessthan <- length(which(cdc$weight < meanweight & cdc$weight > (meanweight - sdweight)))
morethan <- length(which(cdc$weight > meanweight & cdc$weight < (meanweight + sdweight)))
equalto <- length(which(cdc$weight == meanweight))

propweight <- (lessthan + morethan + equalto)/20000

```
The proportion of weights within one standard deviation of the mean is `r propweight`
