...
```{r hist-men, eval = TRUE}
hist(mdims$hgt, main = "Histogram of men's heights", xlab = "Height (cm)")
```

**Histogram of women's heights**

```{r hist-women, eval = TRUE}
hist(fdims$hgt, main = "Histogram of women's heights", xlab = "Height (cm)")
```
**The distributions are similarly bell-shaped.  The spread of the distribution of men's heights is greater than that of the women.**
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
```{r sim_norm_qq, eval=TRUE}
qqnorm(sim_norm)
qqline(sim_norm)
```
**For the simulated data, most points fall on the line.  This is quite similar to the real data.**
...
```{r sim_norm_qqnormsim, eval=TRUE}
qqnormsim(sim_norm)
```
** The plots of simulated data looks very similar to the real data which indicates that heights of females can be approximated to a normal distribution.**

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
  
**First a normal probability plot of female weights will be plotted**

```{r fweight_qq, eval=TRUE}
qqnorm(fdims$wgt)
qqline(fdims$wgt)
```
It seems as though the majority of point fall along the line.

```{r fweight_qqnormsim, eval=TRUE}
qqnormsim(fdims$wgt)
```

These plots are all indicating that female weight can be approximated to normal distribution.
...
* * *
**Q1 - What is the probability that a female is shorter than 5 feet (approx 152 cm)?**

_Theoretical_
```{r fivefeettheory, eval=TRUE}
pnorm(q = 152, mean = fhgtmean, sd = fhgtsd)
```

_Empirical_
```{r fivefeetempirical, eval=TRUE}
sum(fdims$hgt < 152) / length(fdims$hgt)
```

**Q2 - What is the probability that a female is heavier than 200 lbs (approx 91 kg)**
...
```{r 91kftheory, eval=TRUE}
1 - pnorm(q = 91, mean = fwgtmean, sd = fwgtsd)
```

_Empirical_
```{r 91kgempirical, eval=TRUE}
sum(fdims$wgt > 91) / length(fdims$wgt)
```

Based on the result it seems as though weight has a slightly stronger normal approximation
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    **This is because those distribution are slightly positively skewed**
...
```{r fknee_qq, eval=TRUE}
qqnorm(fdims$kne.di)
qqline(fdims$kne.di)
```
The data is right-skewed.

To confirm see historgram:

```{r hist-knee, eval=TRUE}
