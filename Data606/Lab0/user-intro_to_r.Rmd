...
##Answer2
_The number of girls baptised has increased over the years.  Baptised followed a decreasing trend between 1640 and 1660 but greatly increased after.  It should be noted that there declines around 1665 and the early 1700s._
...
##Answer3
```{r plot-prop-boys-over-time, eval=TRUE}
plot(arbuthnot$year, arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), type = "l")
```
_The proportion of boys baptised over time has fluctuated with the highest proportion around 1660 and the lowest in the early 1700s._
...
```
_The dataset includes data for years between `r upper` and `r lower`_

```{r dim-present, eval=TRUE}
dim(present)
```
```{r colnames-present, eval=TRUE}
names(present)
```
...
##Answer
```{r view-boys-present, eval=TRUE}
present$boys
```

```{r view-girls-present, eval=TRUE}
present$girls
```

_The numbers are far greater in the presentt.  In the past the number were in the thousands, but in the present the number are in the millions._
...
##Answer
```{r calc-ratio-boys-to-girls-present, eval=TRUE}
present$boys / present$girls
```

```{r plot-boy-to-girl-ratio-present, eval=TRUE}
plot(present$year, (present$boys / present$girls), type = "l")
```
_The boy to girl ratio is decreasing generally.  A boy to girl ratio of more than 1 indicated that the trend of more boys born hold true in the US. To confirm:_

```{r boys-more-than-girls-present, eval=TRUE}
present$boys > present$girls
```
_The amount of boys born exceed the amount of girls born for all years selected _
...
```{r max-total-present, eval=TRUE}
maxpresent <- present$year[which.max(present$boys+present$girls)]
```

_The year with most births is `r maxpresent`_


**Lab Completed by NNedd**
