...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
  ...
  **A streak of length 1, indicates only one Hit followed by a miss.  A streak of length 0 is a miss.**
...
```{r mode-and-max, eval = TRUE}
kobe_table <- table(kobe_streak)
kobe_mode <- names(kobe_table)[which.max(kobe_table)]
kobe_max <- max(kobe_streak)
```
    
**The distribution is positively skewed, with his most typical streak length being `r kobe_mode`  His longest streak was `r kobe_max`.**
...
```{r table-unfair-coin, eval=TRUE}
sum(sim_unfair_coin == "heads")
```

**Ten (10) flips result in heads**
...
```{r plot-sim-streak, eval = TRUE}
sim_table <- table(sim_streak)
sim_mode <- names(sim_table)[which.max(sim_table)]
sim_max <- max(sim_streak)
barplot(sim_table)
```
**This distribution is also positively skewed with the most typical streak being `r sim_mode`.  However, the longest streak in this case is `r sim_max`.**
...
**The distribution should be somewhat similar.  Since the draws are independent the distribution should not vary too much from sample to sample however it is unlikely to be exactly the same since the same Hit/Miss combination will not necessarily be generated**
...
**It seems as though Kobe Bryant's streak length are similar in distribution to the distribution of the simulated shooter.  As a result, there is no evidence that the hot hand model fits Kobe's shooting patterns.  This is because since the simulated shooter was modelled on an independent shot model (and therefore not the hot hand model) it would be expected that it would exhibit a different distribution from Kobe's if his shots followed hot hand.**
